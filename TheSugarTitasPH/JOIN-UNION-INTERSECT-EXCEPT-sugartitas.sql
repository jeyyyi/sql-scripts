...
FROM customer C JOIN orders O ON C.Customer_ID= O.Customer_ID
				JOIN order_line L ON O.Order_ID = L.Order_ID WHERE Quantity > 1 
				AND Order_Date BETWEEN '2019-03-01' AND '2019-03-31';

-- 3. List the ID, full name, the contact number, and the full address of customer who have ordered already.
--    The customer's order id and the date of order should be displayed too. If the customer ordered more than once, 
--    his/her name should also appear in the list more than once.
SELECT C.Customer_ID, CONCAT(First_Name, " ", Last_Name) AS 'Full Name', Contact_No, 
	   CONCAT(House_No, ", ", Street, ", ", Brgy, ", ", City) AS 'Full Address', 
        Order_ID, Order_Date
FROM customer C JOIN orders O ON C.Customer_ID = O.Customer_ID
ORDER BY C.Customer_ID;
...
-- Find the total amount of each customer would be paying per transactions, then display only those who will pay the amount of 500 php or above.
SELECT C.Customer_ID, O.Order_ID, CONCAT(First_Name, " ", Last_Name) AS Full_Name, 
	   Product_Name, Price, Quantity, (Quantity*Price) as TOTAL 
FROM customer C JOIN orders O ON C.Customer_ID = O.Customer_ID
				JOIN order_line L ON O.Order_ID = L.Order_ID
...
-- Display only those who live in Caloocan and availed Skin Care or Clothing products. 
-- Sort the records by the customer_id.
...
FROM Customer C JOIN Orders O
	ON C.Customer_ID = O.Customer_ID
		...
		ON O.Order_ID = OL.Order_ID
            ...
            WHERE ((City = 'Caloocan') AND (Category = 'Skin Care' OR Category = 'Clothes'))
...
-- EXCEPT / NOT IN
-- 1. List all the records of those customers who haven't ordered any products yet. 
SELECT *
FROM customer
WHERE Customer_ID NOT IN (SELECT Customer_ID FROM orders);

-- 2. List all the records of those product that has not been ordered by any customer. 
SELECT *
FROM product
WHERE Product_ID NOT IN (SELECT Product_ID FROM order_line);

-- 3. List all the records of those customers who did not ordered in the month of January 2019
SELECT *
FROM customer
WHERE Customer_ID NOT IN(SELECT Customer_ID FROM orders WHERE Order_Date BETWEEN '2019-01-01' AND '2019-01-31');

-- 4. List all the records of customers except those customers who chose COD as their Mode of Payment. 
-- Customers who haven't ordered anything yet can appear on the list.
SELECT *
FROM Customer
WHERE Customer_ID NOT IN (SELECT Customer_ID FROM Orders WHERE MOP = 'COD')
ORDER BY Customer_ID;
...
-- 3. List all the records from orders table. 
-- Display only those orders ordered with 1 product only in the last 2 weeks of February 2019.
SELECT * 
FROM Orders 
WHERE (Order_Date BETWEEN '2019-02-14' AND '2019-02-28') 
	AND Order_ID IN 
	(SELECT Order_ID FROM Order_Line WHERE Quantity = 1);
